...
public class ProtocolNetwork extends MbusNetwork {
    ...
    public ProtocolNetwork(Model owner, String name, boolean showInReport, boolean showInTrace, int slaveNum,
                           int powerNoiseNodePercentage, int variability, int mediumDegreee, int noiseRange, long lasting, int packetDestinationMax) {
        super(owner, name, showInReport, showInTrace, slaveNum, powerNoiseNodePercentage, variability, mediumDegreee,noiseRange, lasting,packetDestinationMax);
    ...
    public static void main(String[] args) throws Exception {
        int slaveNum;
        int variability;
        int powerNoiseNodePercentage = 0;
        int mediumDegree;
        ...
        if (args.length == 6){
	        ...
	        powerNoiseNodePercentage = Integer.parseInt(args[1]);
	        variability = Integer.parseInt(args[2]);
                noiseEdgesNodePercentage = Integer.parseInt(args[3]);
            ...
            System.out.println("Which is the noise probability impact? (for nodes)" );
        ...
        ProtocolNetwork protocol = new ProtocolNetwork(null, "Protocol network first variant",
                true, true, slaveNum, powerNoiseNodePercentage, variability, mediumDegree, noiseEdgesNodePercentage, lasting, packetDestinationMax);
        ...
        if (Stats.masterSentMessage != 0){
        	/* System.out.println("The weight's average in the header is: " + (protocol.headerSum / protocol.messagesSent) +
        	 ...
        	 System.out.println("Slaves number:"+slaveNum);
             System.out.println("Edges number for each node:"+mediumDegree+ "%");
             System.out.println("Noise parameters:");
             System.out.println("Noise changes every "+variability+" packets (frequency)"); // every variability packet
             System.out.println("Noise impacts only "+powerNoiseNodePercentage+"% of the nodes in the network");
             System.out.println("Foreach node, noise impacts "+noiseEdgesNodePercentage+"% of edges");
