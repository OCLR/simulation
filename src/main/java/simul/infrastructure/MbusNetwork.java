...
public class MbusNetwork  {
    ...
    private SimpleWeightedGraph<MasterGraphNode, DefaultWeightedEdge> createGraphFromFile(String filename)  throws IOException {
        ...
        int NumberOfNodes = 0;
        int node;
        SimpleWeightedGraph<MasterGraphNode, DefaultWeightedEdge> networkGraph = new SimpleWeightedGraph<>(DefaultWeightedEdge.class);
        // for each string.
        for (int i = 0; i < adlist.length; i++) {
            ...
            if (Snodes.length == 0){
                Logger.error("Empty line "+i);
                continue;
            }

            if (Snodes.length == 1){
                Logger.error("Line "+i+ " with only one element");
                continue;
            }
